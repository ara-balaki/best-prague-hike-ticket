...
async function main() {
  if (!noDownload) {
    ...
    execSync(`curl -L "${GTFS_URL}" -o "${TMP_DIR}/PID_GTFS.zip" --progress-bar`, { stdio: 'inherit' });
  ...
  for (const [hub, ids] of stopIdsByHub) {
    ...
    if (ids.size === 0) console.warn(`  ⚠  No stop_ids found for "${hub}" — check the name`);
  }

  const anyHubFound = [...stopIdsByHub.values()].some((s) => s.size > 0);
  if (!anyHubFound) throw new Error('No hub stop_ids found at all. Check hub names.');
  ...
  function flushTrip() {
    ...
    for (const [, hubIdx] of hubPositions) {
      ...
      for (let j = hubIdx + 1; j < buffer.length; j++) {
        ...
        if (prev === undefined || minutes < prev) minMinutesByName.set(name, minutes);
      }
    }

    tripsProcessed++;
    if (tripsProcessed % 100000 === 0) {
      console.log(`  ${tripsProcessed.toLocaleString()} trips scanned, ${tripsWithHub.toLocaleString()} contained a hub`);
    }
  }
  ...
  console.log(`  ${tripsWithHub.toLocaleString()} trips contained at least one hub`);
  console.log(`  ${minMinutesByName.size} unique stops reachable directly from any hub`);
  ...
  console.log(`Augmented ${augmented}/${data.stops.length} stops with journeyMinutes`);
